...
public class DistributionUpdater {
    public static Distribution UpdateDistribution(Distribution distribution,Double newScore) throws PerformanceStatisticsException {
        ...
        if ((distribution == null) || (newScore == null)){
            ...
            distribution.helper3 = ((term1 * delta_n2 * (tmpN*tmpN-3*tmpN+3))+(6*delta_n2*distribution.helper1)-(4*delta_n*distribution.helper2));
            distribution.helper2 = ((term1 * delta_n * (tmpN - 2)) - (3 * tmpN * delta_n * distribution.helper1));
            distribution.helper1 = term1;

            distribution.skewness = (Math.sqrt((double)tmpN) * distribution.helper2/ Math.pow(distribution.helper1, 1.5));
            distribution.kurtosis = (((double)tmpN)*distribution.helper3 / (distribution.helper1*distribution.helper1) - 3.0);

            distribution.normal = DistributionAssumptionChecker.processNormalityAssumption(distribution);
        }
